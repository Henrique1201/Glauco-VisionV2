      ...
      {analysisResult ? (
        <main className="max-w-5xl mx-auto px-6 py-8">
          <div className="mb-8 flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-bold text-gray-900">Segmentação Concluída</h2>
              <p className="text-gray-600">O disco óptico foi isolado para a IA processar.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              <div className="p-4 border-b border-gray-200 font-semibold text-gray-900 bg-gray-50">Imagem Original</div>
              <div className="p-6 flex justify-center bg-gray-100 min-h-[300px] items-center">
                <img src={`http://localhost:8000${analysisResult.image_path}`} alt="Original" className="max-w-full max-h-[400px] rounded object-contain shadow" />
              </div>
            </div>

            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              <div className="p-4 border-b border-gray-200 font-semibold text-gray-900 bg-gray-50">Segmentação (Disco Óptico)</div>
              <div className="p-6 flex justify-center bg-gray-100 min-h-[300px] items-center">
                <img src={`http://localhost:8000${analysisResult.segmented_image_path || analysisResult.image_path}`} alt="Segmentada" className="max-w-full max-h-[400px] rounded object-contain shadow" />
              </div>
            </div>
          </div>

          <div className="flex justify-end">
            <Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-6 text-lg rounded-xl shadow-lg" onClick={() => navigate(`/results?id=${analysisResult.id}`)}>
